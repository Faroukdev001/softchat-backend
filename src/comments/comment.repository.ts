...
export class CommentRepository extends Repository<CommentEntity> {
    ...
    async createComment(createCommentDto: CreateCommentDto, user: User, post: Post): Promise<CommentInfoDto> {

        const { content, parentCommentId, parentCommentAuthor } = createCommentDto;

        const createdAt = moment().tz('Africa/Lagos').format('YYYY-MM-DD HH:mm:ss.SSS'); // You can set your time zone here
        const date = new Date(createdAt);

        const comment = this.create({
            content,
            type: createCommentDto.parentCommentId == null ? CommentType.Comment : CommentType.Reply,
            parentCommentId: parentCommentId === undefined ? undefined : parentCommentId,
            parentCommentAuthor,
            post,
            user,
            createdAt: date,
    ...
    async updateComment(updateCommentDto: UpdateCommentDto, user: User): Promise<CommentInfoDto> {
        ...
        const comment = await this.findOne({ where: { id }, relations: ['user', 'post', 'childComments'] });
        ...
        // Handle the case when comment is not found
